Current detailed authority: `docs/CURRENT_STATE_AND_GAPS.md`.

This file is a short release-status summary. Use `CURRENT_STATE_AND_GAPS.md` for reviewer-facing detail on proof levels, partial support, fragile areas, blockers, and the definition of complete.
...
- Provider setup distinguishes `missing`, `invalid`, `timeout`, `quota/rate limit`, `connectivity`, and `unknown` outcomes for supported cloud providers, OpenAI-compatible endpoints, and Ollama.
...
- Full live-provider paths require a reachable configured provider. No-key onboarding remains available through local deterministic fallback.
...
- ATS score is advisory and local-first; the selected AI provider is used only for qualitative feedback when available. It is not employer-certified and not an employer ATS guarantee.
...
- Make `doctor.mjs` drive stricter capability-based safe modes for browser scraping, OCR, AI provider reachability, and local storage.
- Run fresh-state manual proofs for no-key local mode, Ollama, and one configured cloud provider with current resume upload, clarification, scan, and populated dashboard validation.
